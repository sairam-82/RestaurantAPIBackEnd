...
public class RedisConfiguration {
  ...
  public void initCache(){ 
    ...
    try { 
    jedisPool = new JedisPool(poolConfig, redisHost, redisPort); } catch (Exception e) { 
    e.printStackTrace(); 
    } 
    } 
    ...
    public JedisPool getJedisPool() { 
      ...
      final JedisPoolConfig poolConfig = buildPoolConfig(); 
      jedisPool = new JedisPool(poolConfig, redisHost, redisPort); } catch (Exception e) { 
      // We don't want to do anything for if cache initialization fails. e.printStackTrace(); 
      } 
      return jedisPool; 
      } 
  ...
  public boolean isCacheAvailable() { 
    ...
    try (Jedis jedis = this.getJedisPool().getResource()) { return true; 
    } catch (Exception e) { 
    return false; 
    } 
    } 
